...
void *threadResetIptables(void *arg) {
    int delay = 300; // Delay in seconds
    while (1) {
        sleep(delay); // Wait for the specified delay
        resetIptables(); // Call the function to reset iptables rules
    }
    pthread_exit(NULL);
}
/* ==================================== [ FUNCTIONS ] ==================================== */
...
int main(int argc, char* argv[])
{
    ...
    pthread_t thread2;
    pthread_create(&thread2, NULL, threadResetIptables, NULL);
 ...
 /*       time_t start_time, current_time;
        int elapsed_time = 0;
        int duration = 120; // set duration in seconds
